...
// ＤＸライブラリのバージョン
#define DXLIB_VERSION 0x306c
#define DXLIB_VERSION_STR _T( "3.06c" )
...
#define DX_GRAPHICSIMAGE_FORMAT_3D_DRAWVALID_ABGR_F16		(14)	// 描画可能ARGB浮動小数点16ビット型カラー
#define DX_GRAPHICSIMAGE_FORMAT_3D_DRAWVALID_ABGR_F32		(15)	// 描画可能ARGB浮動小数点32ビット型カラー
#define DX_GRAPHICSIMAGE_FORMAT_3D_NUM						(16)
#define DX_GRAPHICSIMAGE_FORMAT_2D							(17)	// 標準( DirectDrawSurface の場合はこれのみ )
#define DX_GRAPHICSIMAGE_FORMAT_R5G6B5						(18)	// R5G6B5( MEMIMG 用 )
#define DX_GRAPHICSIMAGE_FORMAT_X8A8R5G6B5					(19)	// X8A8R5G6B5( MEMIMG 用 )
#define DX_GRAPHICSIMAGE_FORMAT_X8R8G8B8					(20)	// X8R8G8B8( MEMIMG 用 )
#define DX_GRAPHICSIMAGE_FORMAT_A8R8G8B8					(21)	// A8R8G8B8( MEMIMG 用 )

#define DX_GRAPHICSIMAGE_FORMAT_NUM							(22)	// グラフィックフォーマットの種類の数
...
#define DX_BASEIMAGE_FORMAT_ABGR_F16		(6)			// 16ビット不動小数点型
#define DX_BASEIMAGE_FORMAT_ABGR_F32		(7)			// 32ビット不動小数点型
...
extern	int			SetApplicationLogSaveDirectory( const TCHAR *DirectoryPath ) ;			// ログファイルを保存するディレクトリパスを設定する
...
extern	const FLOAT4 *GetConstDefaultParamFToShader( const TCHAR *ConstantName, int ShaderHandle ) ;	// 指定の名前を持つ浮動小数点定数のデフォルトパラメータが格納されているメモリアドレスを取得する
...
extern	int			CreateGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																															// GraphImageデータからグラフィックハンドルを作成する
...
extern	int			ReCreateGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int GrHandle, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;																											// GraphImageデータから既存のグラフィックハンドルにデータを転送する
...
extern	int			CreateDivGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;													// GraphImageデータから分割グラフィックハンドルを作成する
...
extern	int			ReCreateDivGraphFromGraphImage( BASEIMAGE *Image, BASEIMAGE *AlphaImage, int AllNum, int XNum, int YNum, int SizeX, int SizeY, int *HandleBuf, int TextureFlag = TRUE , int ReverseFlag = FALSE ) ;													// GraphImageデータから既存の分割グラフィックハンドルにデータを転送する
...
extern	int			GetDrawValidAlphaChannelGraphCreateFlag( void ) ;					// 描画可能なαチャンネル付き画像を作成するかどうかのフラグを取得する
...
extern	int			GetDrawValidFloatTypeGraphCreateFlag( void ) ;						// 描画可能な浮動小数点型の画像を作成するかどうかのフラグを取得する
...
extern	int			GetUseGraphBaseDataBackup( void ) ;									// 画像を作成する際に使用した画像データのバックアップをしてデバイスロスト時に使用するかどうかのフラグを取得する
